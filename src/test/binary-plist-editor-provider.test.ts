...
suite('BinaryPlistEditorProvider', () => {
  ...
  test('registers the custom editor provider', async () => {
		const extension = vscode.extensions.getExtension('dnicolson.binary-plist');
		assert.ok(extension, 'Extension should exist');
		assert.ok(extension!.isActive, 'Extension should be active and have registered the provider');
  });
